...
import string
import logging
l = logging.getLogger("maeg.trace_additions")
l.setLevel("DEBUG")
...
class ChallRespInfo(SimStatePlugin):
    ...
    def prep_tracer(tracer, format_infos=None):
        ...
        state = path.state
        # break point
        # call exit_hook right before exit happens
        state.inspect.b(
            'exit',
            simuvex.BP_BEFORE,
            action=exit_hook
        )
        # call syscall right before exit happens
        state.inspect.b(
            'syscall',
            simuvex.BP_AFTER,
            action=syscall_hook
        )
        # call constraint_hook right before constraints happens
        state.inspect.b(
            'constraints',
            simuvex.BP_BEFORE,
            action=constraint_hook
        )
        # current state has chall_resp_info object

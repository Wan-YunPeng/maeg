...
def main():
    ...
    '''
    project = angr.Project("printf_test", load_options={'auto_load_libs':False})
    '''
    ...
    '''
    addrPrintf = getFuncAddress('printf', plt=True)
    addrBadFunc = getFuncAddress('badFunc')
    '''
    ...
    '''
    def check(p):
        if (p.state.ip.args[0] == addrPrintf):    # Ensure that we're at printf
            '''
            ...
            # import ipdb
            # ipdb.set_trace()
            BV_printfSrc = p.state.memory.load( p.state.regs.rsi, len(argv[2]) )
            '''
            ...
            '''
            printfSrc = p.state.se.any_str( BV_printfSrc )
            '''
            ...
            '''
            return True if argv[2] in printfSrc else False
        else:
            '''
